...
// s3Upload will accept a path and name to upload to S3
function s3Upload(filePath, fileName) {
    // if a name is not given by the user, 'default' will be used instead
    const name = fileName || 'default'
...
// call the function with the third and fourth arguments passed form the command line
s3Upload(process.argv[2], process.argv[3])
    // console.log the data if the upload resolves
    .then(console.log)
    .catch(console.error)